  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        if (res.status === 200) {
          const rawData = Array.isArray(res.data) ? res.data : (res.data?.courses || []);

          // Normalize data structure for CourseCard
          const normalizedCourses = rawData.map((c: any) => {
            ...
            // Safely parse JSON strings for syllabus if they were stored as such
            let syllabus = c.syllabus;
            try { if (typeof syllabus === 'string') syllabus = JSON.parse(syllabus); } catch (e) { syllabus = []; }

            return {
              id: c._id,
              image: getImageUrl(c.thumbnail || c.image),
              title: c.title,
              shortDescription: c.shortDescription, // ✅ fixed
              fullDescription: c.shortDescription || c.fulldescription,
              ...
              instructorId: (c.instructor && typeof c.instructor === 'object'
                ? c.instructor._id
                : c.instructor) ||
                "1",
            };
